...
const Resources = () => {
  ...
  return (
    <section id="recursos" className="zen-section py-24 md:py-32 bg-[var(--zen-warm-sand)] zen-paper-texture">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto mb-20">
          <div className="bg-[var(--zen-cream)] rounded-[var(--zen-radius-2xl)] p-10 md:p-14 border border-[var(--zen-border)] shadow-[var(--zen-shadow-medium)]">
            {/* Label */}
            <div className="flex justify-center mb-8">
              <span className="inline-flex items-center px-5 py-2 rounded-full text-sm font-medium bg-[var(--zen-soft)] text-[var(--zen-muted)]">
                {t('resources.featuredLabel')}
              </span>
            </div>

            {/* Title */}
            <h3 className="text-2xl md:text-3xl font-light text-[var(--zen-deep)] text-center mb-10 tracking-wide">
              {t('resources.featuredTitle')}
            </h3>

            {/* Content */}
            <div className="space-y-6 mb-12">
              <p className="text-[var(--zen-muted)] leading-relaxed text-lg">
        ...
        {/* Editorial note */}
        <p className="text-center text-[var(--zen-moss)] text-sm italic mb-16">
        ...
        <div className="mb-20">
          <div className="text-center mb-12">
            <h3 className="text-2xl font-light text-[var(--zen-deep)] mb-3 tracking-wide">
              {t('resources.podcastSectionTitle')}
            </h3>
            <p className="text-[var(--zen-muted)] text-lg">
            ...
            {episodes.map((episode, index) => (
              <Card key={index} className="flex flex-col h-full">
                <div className="flex items-center justify-center w-16 h-16 rounded-full bg-[var(--zen-sand)] mb-6">
                  <svg className="w-8 h-8 text-[var(--zen-moss)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
                  </svg>
                </div>
                <h4 className="text-lg font-medium text-[var(--zen-deep)] mb-3">
                  {episode.title}
                </h4>
                <p className="text-[var(--zen-muted)] text-sm leading-relaxed flex-grow">
                  {episode.description}
                </p>
                <div className="mt-6">
                  {episode.spotifyUrl && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {resources.map((resource) => (
            <Card key={resource.id} className="flex flex-col h-full">
              ...
              {resource.id === 'podcast' && (
                <p className="text-[var(--zen-moss)] text-xs italic mt-4">
